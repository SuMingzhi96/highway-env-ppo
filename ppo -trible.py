	...
	def rollout(self):
		...
		while t < self.timesteps_per_batch:
			...
			for ep_t in range(self.max_timesteps_per_episode):
				...
				batch_obs.append(obs[1:20])

				# Calculate action and make a step in the env. 
				# Note that rew is short for reward.
				action, log_prob = self.get_action(obs[1:20])
				...
				# 	# print("add noise")
				# 	noise = np.random.normal(loc=0.0, scale=0.007, size=None)
				# # print("noise", noise)
				# 	action = action + noise
